...
(function (global, factory) {
  ...
  var fromXMLDocument = function (data) {
    ...
    for (
      ...
      var tmpParam = {
        ...
        nicoruCount: Number(item.getAttribute("nicoruCount")) || 0,
        mail: [],
        user_id: -1,
        layer: -1,
      };
  ...
  var parseFont = function (font, size) {
    switch (font) {
      ...
      case "mincho":
        return ""
          .concat(config.fonts[font].weight, " ")
          .concat(size, "px ")
          .concat(config.fonts[font].font);
      default:
        return ""
          .concat(config.fonts.defont.weight, " ")
          .concat(size, "px ")
  ...
  var getStrokeColor = function (comment) {
    if (comment.strokeColor) {
      ...
      if (length_1 === 3 || length_1 === 6) {
        return "rgba("
          .concat(hex2rgb(comment.strokeColor).join(","), ",")
  ...
  var HTML5Comment = (function () {
    ...
    HTML5Comment.prototype.measureText = function (comment) {
      var widthLimit = getConfig(config.commentStageSize, false)[
      ...
      if (
        ...
        comment.charSize =
          comment.charSize * (lineHeight_1 / comment.lineHeight);
  ...
  var FlashComment = (function () {
    ...
    FlashComment.prototype.measureText = function (comment) {
      ...
      var height =
        ...
        this.scale;
      if (comment.loc !== "naka") {
        var widthLimit = getConfig(config.commentStageSize, true)[
          comment.full ? "fullWidth" : "width"
        ];
    ...
    FlashComment.prototype.getTextImage = function () {
      ...
      var lastFont = this.comment.font,
        leftOffset = 0,
        lineCount = 0;
  ...
  var processJumpScript = function (vpos, lastVpos) {
    ...
    for (var _i = 0, _a = nicoScripts.jump; _i < _a.length; _i++) {
      var range = _a[_i];
      var vposInRange = range.start < vpos && (!range.end || vpos < range.end),
        lastVposInRange =
  ...
  var executeEvents = function (eventName, event) {
    for (
      var _i = 0, handlerList_1 = handlerList;
      _i < handlerList_1.length;
      _i++
    ) {
  ...
  var NiconiComments = (function () {
    function NiconiComments(canvas, data, initOptions) {
      ...
      setPlugins(
        config.plugins.map(function (val) {
    ...
    NiconiComments.prototype.preRendering = function (rawData) {
      ...
      if (options.keepCA) {
        //rawData = NicoruLayer(rawData);
        rawData = changeCALayer(rawData);
      }
      this.getCommentPos(
        rawData.reduce(function (pv, val) {
    ...
    NiconiComments.prototype.addComments = function () {
      ...
      var _loop_1 = function (comment) {
        if (comment.invisible) return "continue";
        if (comment.loc === "naka") {
          var posY = 0;
          var beforeVpos =
            Math.round(-288 / ((1632 + comment.width) / (comment.long + 125))) -
            ...
            while (isChanged && count < 10) {
              ...
              for (var j = beforeVpos; j < comment.long + 125; j++) {
                ...
                if (
                  ...
                  var collision =
                    (_b = this_1.collision.left[vpos]) === null || _b === void 0
                      ? void 0
    ...
    NiconiComments.prototype.drawCanvas = function (vpos, forceRendering) {
      ...
      if (
        ...
        ((_b =
          (_a = this.timeline[this.lastVpos]) === null || _a === void 0
            ? void 0
