...
(function (global, factory) {
  ...
  var NiconiComments = (function () {
    function NiconiComments(canvas, data, initOptions) {
      ...
      let fsChange = options.size * 0.01;
      c_stroke = options.stroke;
      c_alpha = this.alpha = options.alpha;

      fontSize = {
        small: {
          default: 47 * fsChange,
          resized: 26.1 * fsChange,
        },
        medium: {
          default: 74 * fsChange,
          resized: 38.7 * fsChange,
        },
        big: {
          default: 110 * fsChange,
          resized: 61 * fsChange,
        },
      };
      this.canvas = canvas;
      ...
      this.context.strokeStyle ? "rgba(0,0,0,0.5)" : "rgba(0,0,0,0.35)";
    ...
    NiconiComments.prototype.drawText = function (comment, vpos) {
      ...
      var posX = (canvasWidth - comment.width_max) / 2,
        posY = comment.posY;
      if (comment.loc === "naka") {
        if (reverse) {
          posX =
      ...
      if (this.showCollision) {
        ...
        lines.forEach(function (_, index) {
          var linePosY =
            ...
            (1 + commentYPaddingTop);
          _this.context.strokeStyle = "rgba(255,255,0,0.35)";
    ...
    NiconiComments.prototype.getTextImage = function (i, preRendering) {
      ...
      if (!context) throw new Error("Fail to get CanvasRenderingContext2D");
      context.strokeStyle = `rgba(0,0,0,${c_stroke})`;
